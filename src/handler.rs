...
impl BitcaskHandler {
    ...
    ///     String::from("/tmp/bitcask"),
    ///     Some(vec![String::from("read_write"), String::from("sync_on_put")])
    /// ).unwrap();
    /// ```
    pub fn open(directory_name: String, options: Option<Options>) -> Result<Self, Box<dyn Error>> {
        Bitcask::open(directory_name, options)
    }
